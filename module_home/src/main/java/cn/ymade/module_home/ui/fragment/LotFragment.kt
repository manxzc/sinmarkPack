...
class LotFragment(val type: Int,val title:String) :BaseFragment<VMLotFragment,FragmentLotListBinding>() {
    ...
    override fun initEvent() {
        LiveDataBus.get().with("searchLot", String::class.java).observe(this,
            { t -> mViewModel?.searchData(t) })

        LiveDataBus.get().with("deleteLotOrSn",String::class.java).observe(this,
            { mViewModel?.reSearchData() })
    }
